...
prompt ============================================
--USO DE FUNCIONES EN PROCEDIMINETOS
...
--ESTA FUNCION AUMENTA EL SALARIO UN 1.05 porciento 
...
CREATE OR REPLACE PROCEDURE prc_aumentar_salarios_it IS
    ...
    Vtotal NUMBER := 0;
BEGIN
-- X es un puntero que se crea en memoria (Una variables) que apunta a los registro de Cur_emp y termina y se destruye
    FOR x IN Cur_emp LOOP
        Vsal2 := fun_calc_aumento(x.salary);
        UPDATE employees e
        SET    e.salary = Vsal2
        ...
        --salir (Solo actualiza un registro si no se coloca IF)
        --exit;
...
prompt ...
prompt ...
CREATE OR REPLACE PROCEDURE prc_control_mascotas(PIdEmpMax NUMBER,PCantMasc NUMBER) IS
    CURSOR Cur_Emp IS
        SELECT employee_id, first_name
        FROM employees
        WHERE employee_id < PIdEmpMax
        ORDER BY 1;
    Vcant NUMBER;
BEGIN
    FOR x IN Cur_Emp LOOP
       -- Vcant := fun_cant_masc_emp(x.employee_id);
        ...
        IF fun_cant_masc_emp(x.employee_id) < PCantMasc THEN
            prc_ins_mascotas(
                'ABC',
                3,
                'Perro',
        ...
        ELSE
            DBMS_OUTPUT.PUT_LINE(
                x.first_name || ' tenia una ' || fun_cant_masc_emp(x.employee_id) || ' mascotas'
            );
        END IF;
    END LOOP;
...
/
show error
EXECUTE prc_control_mascotas(110,3);
...
BEGIN
    FOR z IN Cur_Emp LOOP
        ...
        Vtotal := Vtotal + 1;
        DBMS_OUTPUT.PUT_LINE('Aumento aplicado a: ' || z.first_name ||' salario actual: '|| z.salary);
    END LOOP;
    COMMIT;
...
--AQUI DEBE DE HABER UN ORDEN DE PREGUNTA
