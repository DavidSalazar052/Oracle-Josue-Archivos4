...
exception
  ...
  -- when zero_divide then
    -- return -3;
    -- dbms_output.put_line('Error: Div por cero ' || Pid);
...
prompt ...
prompt ...
create or replace function FUN_una_mascota(Pid number) return varchar2 is
  VNombre varchar2(50);
  Vtemporal number;
begin
  Vtemporal := 1/Pid;
...
exception
  when zero_divide then
    dbms_output.put_line('Aviso: Div por cero para EMP ' || Pid);
    return 'div x cero';
  when no_data_found then
