...
--EL TRIGGER SE ACTIVA EN EL MOMENTO EN QUE SE CREA
--TAMBIEN SE PUEDE DESACTIVAR UN TRIGGER

PROMPT ==== Antes del UPDATE
select * from personas order by id;

PROMPT ====DESACTIVAMOS TRIGGER===========
--DESACTIVAR TRIGGER
alter TRIGGER personas_trg_bur disable;
select * from personas order by id;

PROMPT ====ACTIVAMOS TRIGGER===========
...
PROMPT ====DROPEAMOS TRIGGER===========
--DROPEAR TRIGGER (SE LO VUELA POR COMPLETO)
...
/
show error
PROMPT REMPLAZAMOS TRIGGER PERSONAS TRG_BUR 
--of nombre es cuando hace update de ese campo (era la anterio)
-- si no colocamos el of {x} es para todos los campos de esa tabla 
...
begin
--manejo de bloque de exceptions
  begin
    Vporcentaje := fun_param_valor(4);
  exception
    when others then
      VPorcentaje := 1.1;
  end;
--raise_app pinta un mensaje y quita la ejecución
  if nvl(fun_param_valor(3),'N') <> 'S' then
  raise_application_error(-20001,'NO SE PERMITEN UPD EN LA TABLA PERSONAS');
 end if;

  if :NEW.salario > :old.salario*VPorcentaje then
    raise_application_error(-20001,'NO SE PERMITEN SALARIOS EN MAS DE UN 10%' || VPorcentaje);
  end if;
end personas_trg_bur;
/
show error
...
values (3,'Permite Update Personas','S');
update personas set salario = 1005 where id = 1;

insert into parametros (id,nombre,valor)
values (4,'Por maximo','1.1');
--values (4,'Por maximo','ABC'); si lo colocaramos asi igual no se caeria porque ya lo pusimos 
--como exception
--rollback;
commit;
select * from parametros;
